...
VAL_SPLIT = 0.3
time_handling = True
dt = 0  # time delay between slices expressed as number of snapshots
...
for i in range(n_slices):
    slice_pos[i] = (ny - 1) * (0.5 + slice_distance * (i - (n_slices - 1) / 2))
...
if not time_handling:
    ...
    uvw2D_sec = np.empty([snapshots, nx, nz, 3*n_slices])
    ...
    # Define empty 2D dataset
    uvw2D_sec = np.empty([snapshots-4*dt, nx, nz, 15])
...
uvw3D_field = np.transpose(uvw3D_field, (0,1,3,2,4))
...
with strategy.scope():
    # Input variables
    input_field = tf.keras.layers.Input(shape=(nx, nz, 3*n_slices))
    ...
    x = tf.keras.layers.Conv2D(16, (3, 3), activation=act, padding='same')(x)
    x = tf.keras.layers.Reshape([int(nx / 2), int(nz / 2), int(ny / 8), 1])(x)
